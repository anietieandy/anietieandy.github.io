...
- **Identify PARQ's With Shared Or Similar Needs To A Given Question**: Some unanswered questions do not have a past resolved question with a shared need. For example, given the unanswered question "How can one win a trip to 2006 FIFA World Cup in Germany?", the following PARQ could be recommended, "How do I buy FIFA 2006 tickets in US?". These two questions do not have a shared need but they do have a similar need, namely "attending the FIFA world cup". My collaborators and I proposed an algorithm that identified PARQ with either a shared need or similar need as a given question. This algorithm has two stages: in stage 1, a PARQ is selected as a candidate similar question to a given question if the question-title section of both questions have a cosine similarity greater than a threshold, and the question-title + question-content of both questions contain one or more common entities, entity variations, or knowledge base anchor phrases. In stage 2, features are extracted and used to score whether the answer to Qpast can be used to satisfy the given question, Qgiven. We demonstrated that in entity-rich question categories, using knowledge base information and applying entity linking to identify and disambiguate named entities finds most of the PARQ's with either a shared or similar need to the given questions.
